...
def train_network():
    ...
    cnn_m = cnn()
    # Create cnn
    cnn_m.model.add(Conv2D(64, kernel_size=7, kernel_initializer='random_uniform', bias_initializer='zeros', padding='same', data_format="channels_last", input_shape=(rows, cols, channels)))
    ...
    #cnn_m.model.add(MaxPooling2D(pool_size=(2, 2)))
    cnn_m.model.add(Conv2D(16, kernel_size=5, kernel_initializer='random_uniform', bias_initializer='zeros', padding='same'))
    #cnn_m.model.add(BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones', moving_mean_initializer='zeros', moving_variance_initializer='ones', beta_regularizer=None, gamma_regularizer=None, beta_constraint=None, gamma_constraint=None))
    cnn_m.model.add(Activation('relu'))
    #cnn_m.model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    history = cnn_m.model.fit(np.array(x), y, epochs=200, batch_size=16, validation_data=(np.asarray(x_val), y_val), shuffle=True)
